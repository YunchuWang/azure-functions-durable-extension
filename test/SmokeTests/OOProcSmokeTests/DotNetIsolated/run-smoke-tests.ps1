...
# Build the app first and select its framework-specific output using -AppDirectory.
# The default is the Release/net8.0 output; the runner never rebuilds the app.
...
if (!(Test-Path (Join-Path $AppDirectory 'worker.config.json') -PathType Leaf)) {
    throw "No built Functions app found at '$AppDirectory'. Build the selected framework and pass its output directory using -AppDirectory."
}
$AppDirectory = (Resolve-Path $AppDirectory -ErrorAction Stop).Path
$funcProcess = $null
...
try {
    Do {
        ...
        if ($funcProcess -eq $null -or $funcProcess.HasExited) {
            if ($funcProcess -ne $null) {
                $funcProcess.Dispose()
                $funcProcess = $null
            }
            Write-Host "Starting the Functions host..." -ForegroundColor Yellow
            Write-Host "Working directory: $AppDirectory" -ForegroundColor Cyan
            ...
            $funcProcess = Start-Process `
                ...
                -PassThru `
                -ErrorAction Stop
            Write-Host "Functions host PID: $($funcProcess.Id)" -ForegroundColor Cyan
        ...
        try {
            if ($funcProcess.HasExited) {
                throw "Functions host process $($funcProcess.Id) exited before the health check."
            }

            # Make sure the Functions runtime is up and running
            $pingUrl = "$baseUrl/admin/host/ping"
